...
export function Card({
  ...
  return (
    <div
      className={`rounded-2xl border border-slate-200/80 bg-white p-5 shadow-[0_1px_2px_rgba(15,23,42,0.05),0_12px_32px_-16px_rgba(15,23,42,0.18)] ${className}`}
    >
      {children}
    </div>
  );
...
export function Stat({
  ...
  value: any;
  sub?: string;
  accent?: 'emerald' | 'teal' | 'amber' | 'rose' | 'indigo';
}) {
...
export function Btn(
  ...
  return (
    <button
      {...rest}
      className={`inline-flex items-center justify-center rounded-xl px-3.5 py-2.5 text-sm font-semibold transition disabled:opacity-50 ${cls} ${className}`}
    />
  );
}
...
export function Input(props: React.InputHTMLAttributes<HTMLInputElement>) {
  return (
    <input
      {...props}
      className={`w-full rounded-xl border border-slate-200 bg-white px-3.5 py-2.5 text-sm outline-none transition focus:border-emerald-500 focus:ring-4 focus:ring-emerald-500/15 ${props.className || ''}`}
    />
  );
}
...
export function Select(props: React.SelectHTMLAttributes<HTMLSelectElement>) {
  return (
    <select
      {...props}
      className={`w-full rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm outline-none focus:border-emerald-500 ${props.className || ''}`}
    />
  );
}
