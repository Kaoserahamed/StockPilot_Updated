...
export function Shell({ children }: { children: React.ReactNode }) {
  ...
  const biz = useQuery({
    queryKey: ['biz'],
    queryFn: async () => (await api.get('/businesses/me')).data,
  });
    ...
    <div className="flex min-h-screen">
      <aside className="sticky top-0 hidden h-screen w-64 shrink-0 flex-col bg-slate-950 p-4 text-slate-200 md:flex">
        <div className="flex items-center gap-2.5 px-1 pb-4">
          <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-gradient-to-br from-emerald-400 to-teal-600 text-lg font-black text-white">
            S
          </div>
          <div>
              ...
              <div className="space-y-0.5">
                {g.links.map((l) => {
                  ...
                  return (
                    ...
                    >
                      <span
                        className={`flex h-6 w-6 items-center justify-center rounded-lg text-[13px] ${active ? 'bg-emerald-500 text-white' : 'bg-white/10'}`}
                      >
                        {l.icon}
                      </span>
                      {l.label}
        ...
        <div className="mt-3 rounded-xl bg-white/5 p-3">
          ...
          <button
            onClick={logout}
            className="mt-2 w-full rounded-lg bg-white/10 px-3 py-1.5 text-xs font-semibold hover:bg-white/15"
          >
            Sign out
          </button>
        </div>
          ...
          <div className="mx-auto flex max-w-6xl items-center gap-2 overflow-x-auto p-2 md:hidden">
            {GROUPS.flatMap((g) => g.links).map((l) => (
              <Link
                key={l.href}
                href={l.href}
                className={`whitespace-nowrap rounded-lg px-3 py-1.5 text-xs font-semibold ${isActive(path, l.href) ? 'bg-emerald-600 text-white' : 'bg-slate-100'}`}
              >
                {l.label}
              </Link>
            ))}
