...
export default function InventoryPage() {
  ...
  const over = useQuery({
    queryKey: ['inv'],
    queryFn: async () => (await api.get('/inventory/overview')).data,
  });
  const txs = useQuery({
    queryKey: ['inv-tx'],
    queryFn: async () => (await api.get('/inventory/transactions')).data,
  });
  const padj = useQuery({
    queryKey: ['inv-price'],
    queryFn: async () => (await api.get('/inventory/price-adjustments')).data,
  });
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        <Card>
          ...
          <div className="mt-3 space-y-2.5">
            ...
            <div className="grid grid-cols-2 gap-2">
              <Field label="New sell price">
                <Input
                  value={form.sell}
                  onChange={(e) => set('sell', e.target.value)}
                  placeholder={cur ? String(cur.selling_price ?? '') : 'optional'}
                />
              </Field>
              <Field label="New cost price">
                <Input
                  value={form.cost}
                  onChange={(e) => set('cost', e.target.value)}
                  placeholder={cur ? String(cur.purchase_price ?? '') : 'optional'}
                />
              </Field>
            </div>
            <Field label="Reason">
            ...
            {cur && (
              <div className="rounded-xl bg-slate-50 px-3 py-2 text-xs text-slate-600">
                Current: qty <b>{cur.quantity}</b> · sell <b>{cur.selling_price}</b> · cost{' '}
                <b>{cur.purchase_price}</b>
              </div>
            )}
          </div>
          ...
          <div className="mt-2 max-h-48 space-y-1.5 overflow-auto text-xs text-slate-600">
            {(padj.data || []).slice(0, 20).map((p: any) => (
              <div key={p.id} className="rounded-lg bg-slate-50 px-2 py-1.5">
                ...
                {p.old_purchase_price !== p.new_purchase_price && (
                  <>
                    {' '}
                    · cost {p.old_purchase_price} → <b>{p.new_purchase_price}</b>
                  </>
                )}
        ...
        <Card className="lg:col-span-2">
                  ...
                  <tbody>
                    {(over.data || []).map((r: any) => (
                      <tr key={r.id} className="border-t border-slate-100">
                        ...
                        <td className="px-3 py-2">{r.selling_price}</td>
          ...
          <div className="mt-2 max-h-64 space-y-1.5 overflow-auto text-xs text-slate-600">
            {(txs.data || []).slice(0, 30).map((t: any) => (
              <div key={t.id} className="rounded-lg bg-slate-50 px-2 py-1.5">
                #{t.id} · product {t.product_id} · {t.quantity_change > 0 ? '+' : ''}
                {t.quantity_change} · {t.tx_type}
              </div>
            ))}
