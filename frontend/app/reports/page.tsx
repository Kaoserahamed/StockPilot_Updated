...
export default function ReportsPage() {
  ...
  const q = useQuery({
    queryKey: ['rep', kind, preset],
    queryFn: async () => (await api.get(`/reports/${kind}?preset=${preset}`)).data,
  });
  ...
  const dl = async (fmt: 'csv' | 'excel' | 'pdf') => {
    ...
    try {
      await downloadBlob(
        `/reports/${kind}?preset=${preset}&format=${fmt}`,
        `${kind}.${fmt === 'excel' ? 'xlsx' : fmt}`
      );
    } catch (e) {
      setMsg(errMsg(e));
    }
  ...
  return (
    <Shell>
      <PageTitle
        title="Reports"
        sub="Sales, inventory, purchases, expenses and profit — with exports."
        right={
          <>
      ...
      <Card>
        <div className="mb-3 flex flex-wrap gap-2">
          <Btn variant="secondary" onClick={() => dl('csv')}>
            Download CSV
          </Btn>
          <Btn variant="secondary" onClick={() => dl('excel')}>
            Download Excel
          </Btn>
          <Btn variant="secondary" onClick={() => dl('pdf')}>
            Download PDF
          </Btn>
        </div>
        {summary && (
          <div className="mb-3 grid gap-2 sm:grid-cols-3">
            {Object.entries(summary).map(([k, v]) => (
              <div key={k} className="rounded-xl bg-slate-50 px-3 py-2 text-sm">
                ...
                <div className="font-extrabold">{String(v)}</div>
              </div>
            ))}
          </div>
        )}
