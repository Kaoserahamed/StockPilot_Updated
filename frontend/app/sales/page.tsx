  ...
  const doReturn = async () => {
    ...
    try {
      const r = await api.post('/returns', {
        ...
        reason: ret.reason,
        items: [{ sale_item_id: Number(ret.item), quantity: Number(ret.qty) }],
      });
      setMsg(`Return recorded · refund ${r.data.refund_amount}.`);
      list.refetch();
    } catch (e) {
      setMsg(errMsg(e));
    }
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        <Card>
          ...
          <div className="mt-3 space-y-2.5">
            ...
            <Field label="Sale item ID">
              <Input
                value={ret.item}
                onChange={(e) => setRet({ ...ret, item: e.target.value })}
                placeholder="from sale details"
              />
            </Field>
            <Field label="Qty">
              <Input
            ...
            <Btn onClick={doReturn} disabled={!saleId || !ret.item || !ret.qty} className="w-full">
              Submit return
            </Btn>
          </div>
        </Card>
                  ...
                  {(list.data || []).map((s: any) => (
                    <tr key={s.id} className="border-t border-slate-100">
                      <td className="px-3 py-2 font-medium">
                        {s.invoice_no} <span className="text-slate-400">#{s.id}</span>
                      </td>
                      <td className="px-3 py-2">{s.total_amount}</td>
                      ...
                      <td className="px-3 py-2 text-right">
                        <button
                          onClick={() =>
                            downloadBlob(`/invoices/${s.id}/pdf`, `${s.invoice_no}.pdf`)
                          }
