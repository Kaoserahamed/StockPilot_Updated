      ...
      <div className="grid gap-4 lg:grid-cols-3">
        <Card>
          ...
          <div className="mt-3 space-y-2.5">
            ...
            <Btn onClick={create} disabled={!f.name || !f.password} className="w-full">
              Add member
            </Btn>
          </div>
        </Card>
          ...
          {(list.data || []).length ? (
            <TableWrap>
              <table className="w-full text-sm">
                ...
                <tbody>
                  {(list.data || []).map((m: any) => (
                    <tr key={m.membership_id} className="border-t border-slate-100">
                      <td className="px-3 py-2 font-medium">{m.user?.name}</td>
                      <td className="px-3 py-2">
                        <Badge tone="blue">{m.role}</Badge>
                      </td>
                      <td className="px-3 py-2">
                        {m.is_active ? <Badge tone="green">active</Badge> : <Badge>off</Badge>}
                      </td>
                      <td className="px-3 py-2 text-right space-x-2 text-xs font-semibold">
                        {m.is_active ? (
                          <button
                            onClick={() => act(m.membership_id, 'deactivate')}
                          ...
                          <button
                            onClick={() => act(m.membership_id, 'activate')}
                        ...
                        <button
                          onClick={() => {
                            const r = prompt('New role (Owner/Manager/Cashier):', m.role);
                            if (r) act(m.membership_id, 'role', { role: r });
                          }}
                        ...
                        <button
                          onClick={() => {
                            if (confirm('Remove member?')) act(m.membership_id, 'delete');
                          }}
