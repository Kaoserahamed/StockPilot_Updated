          ...
          <div className="mt-3 space-y-2 text-sm">
            <div className="flex justify-between">
              ...
              <span className="font-bold">
                {d?.product_count ?? '—'} / {d?.product_limit ?? '—'} ({d?.product_usage_pct ?? 0}%)
              </span>
            </div>
            <div className="flex justify-between">
              <span>Team</span>
              <span className="font-bold">
                {d?.employee_count ?? '—'} / {d?.employee_limit ?? '—'} (
                {d?.employee_usage_pct ?? 0}%)
              </span>
            </div>
          </div>
        </Card>
          ...
          <Card key={p}>
            ...
            <div className="mt-1 text-sm text-slate-500">
              {p === 'free'
                ? '100 products · 5 team'
                : p === 'basic'
                  ? '1,000 products · 25 team'
                  : '100k products · 1k team'}
            </div>
            <Btn
              variant={d?.plan === p ? 'ghost' : 'primary'}
              disabled={d?.plan === p}
              onClick={() => change(p)}
              className="mt-3 w-full"
            >
              {d?.plan === p ? 'Current plan' : `Switch to ${p}`}
            </Btn>
          </Card>
        ))}
      </div>
