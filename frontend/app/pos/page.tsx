...
export default function PosPage() {
  ...
  const search = useQuery({
    queryKey: ['pos', q],
    queryFn: async () => (await api.get(`/pos/search?q=${encodeURIComponent(q || 'a')}`)).data,
    enabled: q.length >= 1,
  });
  ...
  const checkout = async () => {
    ...
    try {
      const r = await api.post('/sales/checkout', {
        ...
        paid_amount: paid ? Number(paid) : total,
        payment_method: 'cash',
      });
      setCart([]);
      setPaid('');
    ...
    <Shell>
      ...
      <div className="grid gap-4 lg:grid-cols-5">
        <Card className="lg:col-span-3">
          <Field label="Search products (name / SKU / barcode)">
              ...
              >
                <div>
                  ...
                  <div className="text-xs text-slate-500">
                    {p.sku} · stock {p.quantity} · {p.selling_price}
                  </div>
                </div>
                <Btn variant="secondary" onClick={() => add(p)}>
                  Add
                </Btn>
              </div>
            ))}
            {q && !(search.data || []).length && !search.isLoading && (
              <Empty title="No matches" sub="Try another name, SKU or barcode." />
            )}
          </div>
        </Card>
        <Card className="lg:col-span-2">
          ...
          {!!cart.length && (
            <div className="mt-3 space-y-2 border-t border-slate-100 pt-3">
              ...
              <Field label="Paid amount (default: full)">
                <Input
                  value={paid}
