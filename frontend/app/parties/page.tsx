...
export default function PartiesPage() {
  ...
  return (
    <Shell>
      <PageTitle
        title="Contacts"
        sub="Customers and suppliers in one place."
        right={
          <div className="flex gap-2">
            ...
            <Btn
              variant={tab === 'suppliers' ? 'primary' : 'secondary'}
        ...
        <div className="grid gap-4 lg:grid-cols-3">
          <Card>
            ...
            <div className="mt-3 space-y-2.5">
              ...
              <Btn
                className="w-full"
                disabled={!s.company_name}
                onClick={async () => {
                  try {
                    await api.post('/suppliers', s);
                    setS({ company_name: '', phone: '' });
                    sups.refetch();
              ...
              <TableWrap>
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {(sups.data || []).map((x: any) => (
                      <tr key={x.id} className="border-t border-slate-100">
                        <td className="px-3 py-2 font-medium">{x.company_name}</td>
                        <td className="px-3 py-2">{x.phone || '—'}</td>
                        <td className="px-3 py-2">{x.outstanding_balance}</td>
                      </tr>
                    ))}
      ...
      <div className="mt-2 text-xs text-slate-400">
        Tip: open a contact to see purchase/sales history via the API (
        <Badge>GET /suppliers/:id/purchases</Badge> <Badge>GET /customers/:id/sales</Badge>).
      </div>
    </Shell>
  );
}
