...
export default function ProductsPage() {
  ...
  const list = useQuery({
    queryKey: ['products', q],
    queryFn: async () => (await api.get(`/products${q ? `?q=${encodeURIComponent(q)}` : ''}`)).data,
  });
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        <Card>
          ...
          <div className="mt-3 space-y-2.5">
            ...
            <Btn onClick={create} disabled={!form.name || !form.sku} className="w-full">
              Add product
            </Btn>
          </div>
        </Card>
        <div className="lg:col-span-2">
          <Card>
            ...
            {list.isLoading ? (
              ...
              <Empty title="No products yet" sub="Add your first product." />
            ) : (
              <TableWrap>
                  ...
                  <tbody>
                    {(list.data || []).map((p: any) => (
                      <tr key={p.id} className="border-t border-slate-100">
                        ...
                        <td className="px-3 py-2">{p.selling_price}</td>
                        <td className="px-3 py-2">{p.quantity_on_hand}</td>
                        <td className="px-3 py-2">
                          {p.is_active ? <Badge tone="green">active</Badge> : <Badge>off</Badge>}
                        </td>
