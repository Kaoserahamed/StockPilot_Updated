...
export default function Dashboard() {
  ...
  return (
    <Shell>
      <PageTitle
        title="Good day — here's your business at a glance"
        sub="Sales, profit, stock and smart highlights for the selected period."
        right={
          ...
          <>
            <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
              <Stat
                ...
                sub={`${data.orders ?? 0} orders`}
              />
              <Stat
                accent="teal"
                label="Gross profit"
                value={fmt(data.gross_profit)}
                sub={`Margin ${margin(data.gross_profit, data.revenue)}`}
              />
              <Stat
                accent="indigo"
                label="Net profit"
                value={fmt(data.net_profit)}
                sub={`After ${fmt(data.expenses)} expenses`}
              />
              <Stat
                accent={
                ...
                label="Stock health"
                value={`${data.inventory?.units ?? 0} units`}
                sub={`${data.low_stock_count ?? 0} low · ${data.out_of_stock_count ?? 0} out`}
              />
            </div>
            <div className="mt-4 grid gap-4 lg:grid-cols-5">
              ...
              <Card className="lg:col-span-2">
                ...
                <div style={{ height: 220 }}>
                  <ResponsiveContainer>
                    <BarChart
                      data={(data.top_products || []).slice(0, 6)}
                      layout="vertical"
                  ...
                  {(data.top_products || []).slice(0, 5).map((t: any) => (
                    <li key={t.product_id} className="flex items-center justify-between py-2">
                      ...
                      <span className="text-slate-500">
                        {t.quantity} sold · {fmt(t.revenue)}
                      </span>
                    </li>
                  ))}
                  {!(data.top_products || []).length && (
                    <li className="py-3 text-sm text-slate-500">No sales yet in this period.</li>
                  )}
                </ul>
              </Card>
            </div>
            <Card className="mt-4 border-emerald-100 bg-gradient-to-br from-emerald-50/70 to-teal-50/40">
              <SectionTitle
                ...
                sub="Automatically spotted from sales, stock and expenses"
              />
              {(insights.data?.insights || []).length ? (
                ...
                <div className="text-sm text-slate-500">
                  Everything looks steady. New highlights will appear here.
                </div>
              )}
            </Card>
          </>
