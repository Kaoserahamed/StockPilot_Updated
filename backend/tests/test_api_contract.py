...
OPENAPI_DOCUMENT = REPO_ROOT / "docs" / "api" / "openapi.yaml"
...
def test_committed_openapi_document_matches_the_running_app() -> None:
    """``docs/api/openapi.yaml`` is a reviewable copy of the live schema.

    Generated by ``python backend/scripts/export_openapi.py`` and asserted here
    so a route, request model or response model change cannot land without the
    documented contract following it.
    """
    assert OPENAPI_DOCUMENT.is_file(), (
        "docs/api/openapi.yaml is missing - regenerate it with "
        "`cd backend && python scripts/export_openapi.py`"
    )
    committed = yaml.safe_load(OPENAPI_DOCUMENT.read_text(encoding="utf-8"))
    assert committed == app.openapi(), (
        "docs/api/openapi.yaml is stale - regenerate it with "
        "`cd backend && python scripts/export_openapi.py`"
    )
