...
def business_insights(db: Session, business_id: int, preset: str = "month") -> dict:
    ...
    if trend:
        ...
        if len(revs) >= 2 and revs[0] > 0:
            ...
            if abs(change) >= 20:
                insights.append(
                    {
                        "type": "sales_change",
                        "title": f"Sales up {abs(change):.1f}% in period"
                        if change > 0
                        else f"Sales down {abs(change):.1f}% in period",
                        "detail": f"From {revs[0]} to {revs[-1]}.",
                    }
                )
        ...
        insights.append(
            {
                "type": "sales_peak",
                "title": "Peak sales day {}".format(peak["period"]),
                "detail": "Revenue {} from {} order(s).".format(peak["revenue"], peak["orders"]),
            }
        )
    if profit["total_expenses"] > 0 and profit["net_revenue"] > 0:
        ...
        if ratio >= 30:
            insights.append(
                {
                    "type": "expense_watch",
                    "title": f"Expenses are {ratio:.1f}% of revenue",
                    "detail": (
                        f"Expenses {profit['total_expenses']} vs net revenue "
                        f"{profit['net_revenue']}."
                    ),
                }
            )
    if top:
        ...
        insights.append(
            {
                "type": "demand",
                "title": "Strong demand: {}".format(best["product_name"]),
                "detail": "{} unit(s), revenue {}.".format(best["quantity"], best["revenue"]),
            }
        )
        ...
        if zeros:
            ...
            insights.append(
                {
                    "type": "slow_mover",
                    "title": f"{len(zeros)} product(s) with no sales",
    ...
    if profit["net_profit"] < 0:
        insights.append(
            {
                "type": "loss",
                "title": "Business ran at a net loss",
                "detail": "Net profit {}.".format(profit["net_profit"]),
            }
        )
    if not insights:
        insights.append(
...
def forecast_demand(
    db: Session, business_id: int, product_id: int | None = None, days: int = 30
) -> dict:
        ...
        out.append(
            {
                ...
                "forecast_days": days,
                "predicted_demand": round(daily * days, 2),
