        ...
        recs.append(
            {
                **row,
                ...
                "needs_reorder": qty > 0,
                "reason": (
                    f"Predicted {row['predicted_demand']} + min {p.min_stock} "
                    f"- on-hand {p.quantity_on_hand}"
                ),
            }
        )
        ...
        if std > 0:
            for t in trend:
                ...
                if abs(z) >= 2:
                    anomalies.append(
                        {
                            "type": "daily_revenue_outlier",
                            "title": "Unusual daily revenue on {}".format(t["period"]),
                            "detail": "Revenue {} (z={:.2f}).".format(t["revenue"], z),
                            "z": round(z, 2),
                            "period": t["period"],
                        }
                    )
    sale_rows = (
        db.query(Sale.id, Sale.invoice_no, Sale.total_amount)
        .filter(Sale.business_id == business_id, Sale.status.in_(ff.REVENUE_STATUSES))
    ...
    if sale_rows:
        ...
        for sid, inv, tot in [s for s in sale_rows if mean > 0 and (s[2] or 0) >= mean * 3][:5]:
            anomalies.append(
                {
                    "type": "large_sale",
                    "title": f"Abnormally large sale {inv}",
                    "detail": f"Total {tot} vs average {mean:.2f}.",
                    "sale_id": sid,
                    "invoice_no": inv,
                }
            )
...
def summarize_report(report_kind: str, report_data: dict) -> str:
    ...
    if report_kind == "profit":
        return (
            "Profit: net revenue {net_revenue} from {orders} order(s); "
            "COGS {cogs}; expenses {total_expenses}; gross {gross_profit}, "
            "net {net_profit}.".format(**report_data)
        ) + tag
    if report_kind == "sales":
        s = report_data.get("summary", report_data)
        return (
            "Sales: {orders} order(s), gross {gross_revenue}, "
            "net {net_revenue} after refunds {refunded}.".format(**s)
        ) + tag
    if report_kind == "expenses":
        s = report_data.get("summary", report_data)
        total = s.get("total_expenses")
        n = len(report_data.get("rows", []))
        return f"Expenses total {total} across {n} record(s)." + tag
    if report_kind == "inventory":
        s = report_data.get("summary", report_data)
        return (
            "Inventory: {units} unit(s) across {skus} SKU(s); cost {cost_value}, "
            "retail {retail_value}.".format(**s)
        ) + tag
