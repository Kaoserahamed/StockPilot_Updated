...
def _rule_answer(question: str, snap: dict) -> str:
    ...
    if any(k in q for k in ("low stock", "running low", "out of stock", "reorder", "inventory")):
        inv = snap["inventory"]
        msg = (
            f"Inventory: {inv['units']} units across {inv['skus']} SKUs "
            f"(cost value {inv['cost_value']}, retail {inv['retail_value']})."
        )
        if snap["top_products"]:
            t = snap["top_products"][0]
            msg += (
                f" Fastest mover: {t['product_name']} ({t['quantity']} sold). "
    ...
    if "customer" in q:
        ...
        c = snap["customers"][0]
        return (
            f"Top customer: {c['customer_name']} spent {c['spent']} across {c['orders']} order(s)."
        )
    if "supplier" in q or "purchase" in q:
        ...
        s = snap["suppliers"][0]
        return (
            f"Top supplier: {s['supplier_name']} supplied {s['purchased']} "
            f"({s['orders']} order(s), outstanding {s['outstanding']})."
        )
    if "expense" in q or "largest" in q:
        return (
            f"Total expenses: {p['total_expenses']}. Gross profit {p['gross_profit']}, "
    ...
    if "profit" in q or "loss" in q or "decrease" in q or "why" in q:
        return (
            f"Revenue (net) {p['net_revenue']} from {p['orders']} order(s); "
    ...
    if "sold" in q or "most" in q or "best" in q or "top product" in q:
        ...
        t = snap["top_products"][0]
        return (
            f"Best seller: {t['product_name']} - {t['quantity']} unit(s), "
            f"revenue {t['revenue']}, est. profit {t['profit']}."
        )
    return (
        f"Sales: {p['orders']} order(s), gross revenue {p['gross_revenue']}, "
...
def maybe_polish_with_gemini(question: str, draft: str, snap: dict) -> str:
    """Optionally rewrite the deterministic answer using Gemini.

    The key is read from typed settings rather than ``os.environ`` so that a
    value placed in ``.env`` is honoured (pydantic-settings loads ``.env`` into
    the Settings object but does not export it to the process environment).

    Returns ``draft`` unchanged when no key is configured or on any error, so
    AI is always optional and never a hard dependency.
    """
