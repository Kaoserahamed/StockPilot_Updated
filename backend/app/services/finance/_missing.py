...
def inventory_value(db: Session, business_id: int) -> dict:
    ...
    row = (
        db.query(
            ...
            func.coalesce(func.sum(Product.quantity_on_hand * Product.purchase_price), 0),
            func.coalesce(func.sum(Product.quantity_on_hand * Product.selling_price), 0),
        )
...
def cogs_summary(db: Session, business_id: int, start, end) -> dict:
    ...
    rows = (
        db.query(
            SaleItem.product_id,
            func.coalesce(func.sum(per * func.coalesce(Product.purchase_price, 0.0)), 0),
    ...
    items = [
        {
            "product_id": r[0],
            "product_name": names.get(r[0]),
            "quantity": int(r[2] or 0),
            "cogs": round(float(r[1] or 0), 2),
        }
