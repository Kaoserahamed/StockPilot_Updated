...
def customer_stats(
    ...
    rows = (
        ...
        .filter(*_sale_range_filters(business_id, start, end), Sale.customer_id.isnot(None))
...
def supplier_stats(db: Session, business_id: int, start: object, end: object) -> list[dict]:
    ...
    rows = (
        db.query(Purchase.supplier_id, func.count(Purchase.id), func.sum(Purchase.total_amount))
        .filter(*flt)
    ...
    if rows:
        ids = [r[0] for r in rows]
        sups = {s.id: s for s in db.query(Supplier).filter(Supplier.id.in_(ids)).all()}
    out = []
        ...
        out.append(
            {
                ...
                "orders": int(cnt or 0),
                "purchased": round(float(purchased or 0), 2),
