...
def subscription_status(db: Session, business_id: int) -> dict:
    ...
    employee_count = (
        ...
        .filter(UserBusiness.business_id == business_id, UserBusiness.is_active.is_(True))
        .count()
    )
    ...
    return {
        ...
        "product_usage_pct": p_pct,
        "employee_usage_pct": e_pct,
        "near_limit": near,
        "at_limit": at,
        "message": msg,
    }
...
def check_can_add_product(db: Session, business_id: int) -> None:
    ...
    if count >= (sub.product_limit or 0):
        raise HTTPException(
            status_code=402,
            detail=(
                f"Product limit reached for '{sub.plan}' plan ({sub.product_limit}). "
                "Upgrade to add more."
            ),
        )
...
def check_can_add_employee(db: Session, business_id: int) -> None:
    ...
    if count >= (sub.employee_limit or 0):
        raise HTTPException(
            status_code=402,
            detail=(
                f"Employee limit reached for '{sub.plan}' plan ({sub.employee_limit}). "
                "Upgrade to add more."
            ),
        )
