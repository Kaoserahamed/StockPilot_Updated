...
def update(
    ...
    c = (
        db.query(Category)
        .filter(Category.id == cat_id, Category.business_id == ctx.business_id)
        .first()
    )
    if not c:
...
def deactivate(
    cat_id: int, ctx: Context = Depends(get_current_context), db: Session = Depends(get_db)
):
    if ctx.role not in ("Owner", "Manager"):
        raise HTTPException(status_code=403, detail="Insufficient permissions")
    c = (
...
def delete(cat_id: int, ctx: Context = Depends(get_current_context), db: Session = Depends(get_db)):
    ...
    count = (
        db.query(Product)
        .filter(Product.category_id == cat_id, Product.business_id == ctx.business_id)
        .count()
    )
    if count > 0:
        raise HTTPException(
            status_code=400,
            detail=f"Cannot delete: {count} product(s) use this category. Reassign first.",
        )
    db.delete(c)
    db.commit()
