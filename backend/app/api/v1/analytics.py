...
def dashboard(
    ...
    low = (
        db.query(Product)
        .filter(
            Product.business_id == ctx.business_id,
            Product.is_active.is_(True),
            Product.quantity_on_hand > 0,
            Product.quantity_on_hand <= Product.min_stock,
        )
        .count()
    )
    out = (
    ...
    return {
        "preset": preset,
        "total_sales": profit["gross_revenue"],
...
def product_perf(
    ...
    return {
        "best_sellers": [r for r in rows if r["quantity"] > 0][:5],
...
def customer_perf(
    ...
    return {
        "top_customers": rows[:5],
        "returning": [r for r in rows if r["orders"] > 1],
        "all": rows,
    }
