...
def update(
    ...
    _check(ctx)
    e = db.query(Expense).filter(Expense.id == eid, Expense.business_id == ctx.business_id).first()
    if not e:
...
def remove(eid: int, ctx: Context = Depends(get_current_context), db: Session = Depends(get_db)):
