...
def create_return(
    ...
    sale = (
        db.query(Sale)
        .filter(Sale.id == payload.sale_id, Sale.business_id == ctx.business_id)
        .first()
    )
    if not sale:
    ...
    items = {i.id: i for i in db.query(SaleItem).filter(SaleItem.sale_id == sale.id).all()}
    refund = 0.0
    lines = []
    for r in payload.items:
        ...
        if r.quantity > avail:
            raise HTTPException(status_code=400, detail=f"Only {avail} returnable for item {si.id}")
...
def list_returns(ctx: Context = Depends(get_current_context), db: Session = Depends(get_db)):
    rows = (
        db.query(SaleReturn)
        .filter(SaleReturn.business_id == ctx.business_id)
