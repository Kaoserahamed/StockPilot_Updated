...
def list_employees(
    ...
    rows = (
        db.query(UserBusiness, User)
        .join(User, User.id == UserBusiness.user_id)
        .filter(UserBusiness.business_id == ctx.business_id)
        .all()
    )
    return [
        EmployeeOut(
...
def create_employee(
    ...
    db.flush()
    m = UserBusiness(
        user_id=user.id, business_id=ctx.business_id, role=payload.role, is_active=True
    )
    db.add(m)
    write_audit(
        db,
...
def deactivate(
    membership_id: int, ctx: Context = Depends(get_current_context), db: Session = Depends(get_db)
):
    _owner(ctx)
    m = (
        db.query(UserBusiness)
        .filter(UserBusiness.id == membership_id, UserBusiness.business_id == ctx.business_id)
        .first()
    )
    if not m:
