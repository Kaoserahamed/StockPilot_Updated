...
from app.services import ai2_service, ai3_service, ai_service
...
def insights(
    ...
    if save:
        ...
        _save(db, ctx.business_id, "insight", f"Insights {preset}", body)
    return out
...
def summarize(
    ...
    if payload.kind == "profit":
        ...
        rows = (
            db.query(__import__("app.models.finance", fromlist=["Expense"]).Expense)
            .filter(
    ...
    rec = _save(db, ctx.business_id, "summary", f"{payload.kind} summary {preset}", summary)
    return {
        "kind": payload.kind,
        "preset": preset,
        "summary": summary,
...
def list_recs(ctx: Context = Depends(get_current_context), db: Session = Depends(get_db)):
    ...
    return (
        ...
        .filter(AIRecommendation.business_id == ctx.business_id)
...
def update_rec(
    ...
    rec = (
        db.query(AIRecommendation)
        .filter(AIRecommendation.id == rid, AIRecommendation.business_id == ctx.business_id)
        .first()
    )
    if not rec:
