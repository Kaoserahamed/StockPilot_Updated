...
def get_sale(
    sale_id: int, ctx: Context = Depends(get_current_context), db: Session = Depends(get_db)
):
        ...
        if left > 0:
            ...
            db.add(
                InventoryTransaction(
                    ...
                    quantity_change=left,
                    tx_type="sale",
                    reason=f"sale #{s.id} cancelled: {payload.reason}",
                    user_id=ctx.user.id,
                    related_id=f"sale:{s.id}",
                )
            )
