...
def sales_report(
    ...
    if exp is not None:
        return exp
    if format == "pdf":
        return _report_pdf(f"Sales Report ({preset})", summary, data, "sales.pdf")
...
def inventory_report(
    ...
    prods = (
        db.query(Product)
        .filter(Product.business_id == ctx.business_id)
...
def purchase_report(
    ...
    if exp is not None:
        return exp
    if format == "pdf":
        return _report_pdf(f"Purchase Report ({preset})", summary, data, "purchases.pdf")
    return {"rows": data, "summary": summary}
...
def expense_report(
    ...
    if exp is not None:
        return exp
    if format == "pdf":
        return _report_pdf(f"Expense Report ({preset})", summary, data, "expenses.pdf")
    return {"rows": data, "summary": summary}
...
def profit_report(
    ...
    if exp is not None:
        return exp
    if format == "pdf":
        return _report_pdf(f"Profit Report ({preset})", summary, data, "profit.pdf")
    return summary
