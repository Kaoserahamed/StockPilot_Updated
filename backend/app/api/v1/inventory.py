...
def low_stock(ctx: Context = Depends(get_current_context), db: Session = Depends(get_db)):
    ...
    return [
        {"id": p.id, "name": p.name, "quantity": p.quantity_on_hand, "min_stock": p.min_stock}
        for p in rows
        if (p.quantity_on_hand or 0) < (p.min_stock or 0) and (p.quantity_on_hand or 0) > 0
    ]
...
def out_of_stock(ctx: Context = Depends(get_current_context), db: Session = Depends(get_db)):
    rows = db.query(Product).filter(Product.business_id == ctx.business_id).all()
    return [
        {"id": p.id, "name": p.name, "quantity": p.quantity_on_hand}
        for p in rows
        if (p.quantity_on_hand or 0) <= 0
    ]
...
def adjust_price(
    ...
    product = (
        db.query(Product)
        .filter(Product.id == payload.product_id, Product.business_id == ctx.business_id)
        .first()
    )
    if not product:
    ...
    if payload.new_selling_price is None and payload.new_purchase_price is None:
        raise HTTPException(
            status_code=422, detail="Provide new_selling_price and/or new_purchase_price"
        )
    ...
    row = PriceAdjustment(
        ...
        old_purchase_price=old_cost,
        new_purchase_price=new_cost,
        reason=payload.reason,
        user_id=ctx.user.id,
    )
    db.add(row)
